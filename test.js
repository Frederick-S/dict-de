...
describe('Parse data', function () {
    ...
    it('Ignore invalid word examples', function (done) {
        query('Name').done(function (data) {
            assert.notEqual('Request error.', data);

            var word = parse(data);

            assert.equal('Name', word.name);
            assert.equal(true, word.valid);
            assert.equal(9, word.explanations.length);
            assert.equal(3, word.examples.length);
